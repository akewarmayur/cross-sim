...
class NumericCore(ICoreInternal):
    ...
    def __init__(
        ...
        self._mapping = CoreMappingParameters(
            weights={"min": gmin, "max": gmax, "percentile": None},
            # TODO:
        ...
        if not self.simulate_parasitics:
        ...
        elif self.xbar_params.array.parasitics.gate_input:
            # Note that gate_input is not physically equivalent to Rp=0 on the
            # input side
            ...
            circuit_solver = mvm_parasitics_gateInput
        else:
