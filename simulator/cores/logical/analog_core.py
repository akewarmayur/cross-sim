...
class AnalogCore(ICore):
    ...
    def set_matrix(
        ...
        if error_mask:
            ...
            if mat_max > (self.max + self._eps) or mat_min < (self.min - self._eps):
                print(mat_max, self.mapping.weights.max)
                print(mat_min, self.mapping.weights.min)
                warnings.warn(
                    (
        ...
        # Only set percentile limits if we're writng the full matrix
            ...
            self._mapping.weights.min = self.min
            self._mapping.weights.max = self.max
        ...
        for row, row_start, row_end in self.row_partition_bounds:
            for col, col_start, col_end in self.col_partition_bounds:
                ...
                if error_mask:
                    ...
                    error_mask_ = logical_utils.set_error_mask(
                        error_mask=emask,
                        row_start=row_start,
                        col_start=col_start,
                        row_end=row_end,
                ...
                self._scale_and_set(
                    matrix=mcopy[row_start:row_end, col_start:col_end],
    ...
    def set_mvm_inputs(self, vector: npt.NDArray):
        ...
        """
        if (
            self.mapping.inputs.mvm._implicitly_initialized
            and self.mapping.inputs.mvm.clipping
        ):
            if (
                vector.max() > self.mapping.inputs.mvm.max
                or vector.min() < self.mapping.inputs.mvm.min
            ):
                warnings.warn(
    ...
    def set_vmm_inputs(self, vector: npt.NDArray):
        ...
        """
        if (
            self.mapping.inputs.vmm._implicitly_initialized
            and self.mapping.inputs.vmm.clipping
        ):
            if (
                vector.max() > self.mapping.inputs.mvm.max
                or vector.min() < self.mapping.inputs.vmm.min
            ):
                warnings.warn(
    ...
    def matvec(self, vec: npt.ArrayLike) -> npt.NDArray:
        ...
        if self.mvm_input_percentile_scaling:
            input_range = self._set_limits_percentile(
                self.mapping.inputs.mvm,
                vcopy,
                reset=True,
            )
            self._mapping.inputs.mvm.min = input_range[0]
            self._mapping.inputs.mvm.max = input_range[1]
    ...
    def vecmat(self, vec: npt.ArrayLike) -> npt.NDArray:
        ...
        if self.vmm_input_percentile_scaling:
            input_range = self._set_limits_percentile(
                self.mapping.inputs.vmm,
                vcopy,
                reset=True,
            )
            self._mapping.inputs.vmm.min = input_range[0]
            self._mapping.inputs.vmm.max = input_range[1]
    ...
    def _scale_and_set(
        ...
        """Internal function to scale weights and set matrix."""
    ...
    def _reset(self):
        ...
        # breaks complex equivalent expansion. This could probably be fixed but
        # it is a sufficiently unusual case that just throw an error for now.

        input_mapping = self.mapping.inputs
        ...
        self.mvm_input_percentile_scaling = input_mapping.mvm.percentile is not None
        self.vmm_input_percentile_scaling = input_mapping.vmm.percentile is not None
