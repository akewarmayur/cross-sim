...
class ICoreLower(ICoreInternal):
    ...
    def __init__(
        ...
        for subcore in self.subcores.values():
            subcore.mapping.inputs.mvm = self.mapping.inputs.mvm.copy()
            subcore.mapping.inputs.vmm = self.mapping.inputs.vmm.copy()

    @property
    @abstractmethod
    def default_mapping(self) -> CoreMappingParameters:
        """Returns the default mapping of the lower core."""
    ...
    def run_xbar_mvm(self, vector: npt.ArrayLike | None = None) -> npt.NDArray:
        ...
        core = _SingleOperationCore(input_mapping=self.mapping.inputs.mvm)
        ...
        if self.mapping.weights != self.default_mapping.weights:
            multiplier = self.mapping.weights.range / self.default_mapping.weights.range
            output *= multiplier
        return output
    ...
    def run_xbar_vmm(self, vector: npt.ArrayLike | None = None) -> npt.NDArray:
        ...
        core = _SingleOperationCore(input_mapping=self.mapping.inputs.vmm)
        for key in self.subcores:
