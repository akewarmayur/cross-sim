...
def add_adc_to_core(
    ...
    if adc_scheme == ConvertingConfiguration.SKIP_CONVERSION:
        ...
        if subcores is None and core.subcores is not None:
            is_lower_core = True
...
def interpret_key(key: str) -> tuple | int:
    ...
    """
    # Try to interpret the key as an integer
    if isinstance(key, (int, tuple)):
        return key
    if not isinstance(key, str):
        raise TypeError(
            "Key is not of a valid type. The expeted type is int, tuple, ",
            f"or str, but your key is type{type(key)}.",
        )

    if re.match("^\\s*-?\\d+\\s*$", key):
        return int(key)
    elif re.match("^\\s*\\(\\s*-?\\d+\\s*,\\s*-?\\d+\\s*\\)\\s*$", key):
