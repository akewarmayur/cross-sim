...
# To optimize performance, solvers for different circuit topologies are
...
def solve_mvm_circuit(
    ...
    """Wrapper that is used to implement a convergence loop around the circuit
    solver.
    ...
    Args:
        ...
        interleaved: whether positive and negative resistors are interleaved in
            the array
        ...
        row_in: inputs passed in through rows, False if inputs passed in through
            columns. In numeric_core row_in = True if MVM, row_in = False if VMM
    """
    ...
    while not solved:
        ...
        try:
            ...
            if params.simulation.relaxation_gamma <= 1e-2:
                raise ValueError("Parasitic MVM solver failed to converge") from e
...
def mvm_parasitics(vector, matrix, params, useMask=False, mask=None, row_in=True):
    """Calculates the MVM result including parasitic resistance, for a
    ...
    while Verr > Verr_th and Niters < Niters_max:
        ...
        # Evaluate overall error; if using SIMD, make sure only to count the
        # cells that matter
        if useMask:
...
def mvm_parasitics_gateInput(
    ...
    Assumes an array topology where input is applied bitwise on the gate, and
    there is no parasitic resistance along the input dimension
        (gate_input = True, input_bitslicing = True)
    ...
    # If input is zero, the device is gated off and cannot conduct current
    # regardless of the drain voltage
...
def mvm_parasitics_interleaved(
    ...
    """Calculates the MVM result including parasitic resistance, for an array
    where the positive and negative resistors in a differential pair are
    connected to the same column. On the other side, the positive resistor is
    connected to +VDD, the negative resistor is connected to -VDD.

    Parasitic resistance is inserted between every pair, and not between the two
    devices in the pair

    Currently this is compatible only with a topology where the input is applied
    bitwise on the gate, and there is no parasitic resistance along a row
    (gate_input = True, input_bitslicing = True)

    Input arguments are the same as mvm_parasitics, except:
        matrix_pos: normalized conductance matrix for positive conductances in
            diff. pair
        matrix_neg: normalized conductance matrix for negative conductances in
            diff. pair
    """
