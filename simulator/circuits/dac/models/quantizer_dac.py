...
from simulator.parameters.core import MappingParameters
...
class QuantizerDAC(IDAC):
    ...
    def __init__(
        ...
        input_mapping_params: MappingParameters,
    ):
        ...
        """
        super().__init__(dac_params, input_mapping_params)
        self.levels = 2**self.bits

        if self.input_mapping_params.percentile is not None:
            if self.signed:
                self.min = -1
                self.max = 1
            else:
            ...
            if self.signed:
                self.min = (
                    2 * self.input_mapping_params.min / self.input_mapping_params.range
                )
                self.max = (
                    2 * self.input_mapping_params.max / self.input_mapping_params.range
                )
            else:
                self.min = (
                    self.input_mapping_params.min / self.input_mapping_params.range
                )
                self.max = (
                    self.input_mapping_params.max / self.input_mapping_params.range
                )
