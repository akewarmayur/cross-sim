...
class BaseParameters:
    ...
    def _recursive_post_init(self, post_init_self: bool = True):
        ...
        for key in self.__dataclass_fields__:
            ...
            elif key == "subcores" and isinstance(value, dict):
                subcore_params = value
                for subcore_param in subcore_params.values():
                    if isinstance(subcore_param, BaseParameters):
                        subcore_param._recursive_validate()
        if post_init_self:
            self.__post_init__()
    ...
    def as_dict(
        ...
        """
        if flat:
            from .utils import flatten_param
        ...
        for field in fields(self):
            ...
            if not field.init and exclude_non_init:
                continue
            if field_name == "subcores" and field_instance is not None:
                # TODO: Remove this temporary patch whenever subcores gets
                # moved to a dict param
                dict_[field_name] = {
                    k: v.as_dict(
                        exclude_non_init=exclude_non_init,
                        exclude_non_repr=exclude_non_repr,
                    )
                    for k, v in field_instance.items()
                }
    ...
    def search(self, key: str) -> dict[str, Any]:
        ...
        """
        from .utils import get_matching_keys

        matching_keys = get_matching_keys(param=self, key=key)
        if ("*" not in key and "?" not in key) and len(matching_keys) == 0:
            raise KeyError(f"No key matches '{key}'")
        ...
        while len(parts):
            try:
                if isinstance(value, dict):
                    # TODO: Remove this when subcore gets moved to dict param
                    try:
                        value = value[parts[0]]
                        parts = parts[1:]
                    except KeyError:
                        k = core_utils.interpret_key(parts[0])
                        value = value[k]
                        parts = parts[1:]
                else:
    ...
    def _get(self, key: list[str]) -> tuple[Any, list[str]]:
        ...
        if key[0] in field_names:
            return getattr(self, key[0]), key[1:]
        else:
            raise AttributeError(
                f"{self.__class__.__name__} object has no attribute {key[0]}",
            )
